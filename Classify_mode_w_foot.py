...
def classify_nn(x_train, y_train, x_test, y_test, n):
    ...
    model.fit(x_train, y_binary, epochs=60, batch_size=10)
...
dataR_cut = np.reshape(dataR[1,:],(1,-1))
dataTR_cut = np.reshape(dataTR[1,:],(1,-1))
des_cut = np.reshape(des[1,:],(1,-1))
for i in range(7):
    dataC_cut = np.append(dataC_cut, dataC[i * 1999 + 500:i * 1999 + 700, :], axis=0)
    dataR_cut = np.append(dataR_cut, dataR[i * 1999 + 500:i * 1999 + 700, :], axis=0)
    dataTR_cut = np.append(dataTR_cut, dataTR[i * 1999 + 500:i * 1999 + 700, :], axis=0)
    des_cut = np.append(des_cut,des[i*1999+500:i*1999+700,:],axis=0)

num_cut = des_cut.shape[0]
t_cut = np.arange(0,num_cut*0.01,0.01)

label_knn_pred = np.zeros((12, num_cut))
label_knn = des_cut
label_svm_pred = np.zeros((12, num_cut))
label_svm = label_knn
label_cnn_pred = np.zeros((12, num_cut))
label_cnn = label_knn
num_classes = 7
...
for j in range(6):
    ...
    #classify using knn
    label_knn_pred[m] = classify_knn(X_train, Y_train, X, Y, num_classes)

    #classify using svm
    label_svm_pred[m] = classify_svm(X_train, Y_train, X, Y, num_classes)

    #classify using cnn
    label_cnn_pred[m] = classify_nn(X_train, Y_train, X, Y, num_classes)
