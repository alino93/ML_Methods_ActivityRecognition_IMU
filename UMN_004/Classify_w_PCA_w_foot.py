...
def classify_nn(x_train, y_train, x_test, y_test, n):
    ...
    model.fit(x_train, y_binary, epochs=40, batch_size=10)
...
def compute_confusion_matrix(label_pred, label_test, n):
    ...
    for i in range(num_test):
        confusion[label_test_pred[i] , label_test[i] ] += 1
...
dataC_cut = dataC[0:50000,:]
dataR_cut = dataR[0:50000,:]
dataTR_cut = dataTR[0:50000,:]
des_cut = np.reshape(des[0:50000,0],(1,-1))

num_cut = des_cut.shape[1]
t_cut = np.arange(0,num_cut*0.01,0.01)

# PCA analysis
X = dataC_cut#np.append(dataC, dataR, axis=1)
Y = des_cut.reshape(-1)#des[:,0].reshape(-1)
t = np.arange(0,(num)*0.00999999999,0.01)

label_knn = Y.reshape(-1,1)
label_svm = Y.reshape(-1,1)
label_cnn = Y.reshape(-1,1)
num_classes = 11

label_classes = ['Stand','Walk','Sit','Turn','Lie down','Bend','Climb','Nfall FWD','Nfall LAT','Nfall BWD','Fall']
